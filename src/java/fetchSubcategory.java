...
public class fetchSubcategory extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try  {
            System.out.println("inside fetch sub cat");
            ...
            Criteria cr = se.createCriteria(SubCategory.class);
            
            cr.add(Restrictions.eq("categoryID", c));
            
            ArrayList<SubCategory> al = (ArrayList<SubCategory>) cr.list();
            System.out.println("bug"+al.size());

            if(al.size() > 0 ){
                for (SubCategory sc : al) {
                    System.out.println(sc.getSubCategoryName());
                    out.println("<option value='"+ sc.getSubCategoryID() +"'>" + sc.getSubCategoryName()+ "</option>");
                }   
            }         
                        tr.commit();
        }
